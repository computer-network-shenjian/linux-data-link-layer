...
/*
Status sender_physical_layer_old(int *pipefd_down, int *pipefd_up) {
    prctl(PR_SET_PDEATHSIG, SIGHUP);
    //fd_set rfds, wfds;
...
Status receiver_physical_layer_old(int *pipefd_down, int *pipefd_up) {
    prctl(PR_SET_PDEATHSIG, SIGHUP);
    fd_set rfds, wfds;
...
void _stop_timer(seq_nr k) {
    timer_list.remove_if([&k](const auto &el) { return el.second == k; } );
}
*/
